...
> * 当对象成活率较高时，会涉及到大量的复制操作，因此本算法就不太适合了。
* 标记-整理算法
> 标记-清除算法，只不过标记完成后不直接回收，而是将存活的对象向一段移动，然后直接清理调边界以外的内存。
* 分代收集算法
> * 新生代对象死亡率高，只有少量存户，可以选用复制算法；
> * 老年代中对象存活率高，没有足够的空间进行分配担保（Survivor无法容纳的对象字节进入老年代，所以需要老年代进行分配担保），就需要使用“标记-清理”或者“标记-整理算法”。
